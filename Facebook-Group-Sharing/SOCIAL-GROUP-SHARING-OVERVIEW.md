...
| **SOCIAL-GROUP-SHARING-OH-DIRECTORY.md** | Ohio groups we're in (**target met**) | Same, OH |
| **SOCIAL-GROUP-SHARING-TN-DIRECTORY.md** | Tennessee groups we're in (**in progress**) | Same, TN |
| **SOCIAL-GROUP-SHARING-MASTER-LINKS.md** | Direct FB URL for all 234 groups, bucketed by state | Jump straight to a group to post; the self-contained link source |
...
- **URL status: DONE — all 234 groups are linked.** `SOCIAL-GROUP-SHARING-MASTER-LINKS.md` in this folder is the complete list of every group with its direct URL, bucketed by state (TX/CA/GA/FL + franchise cluster). Florida also carries its links inline in its own directory. Every new join going forward gets logged with its URL. (The live `facebook.com/groups/joins/` page stays the source of truth if the master file ever drifts.)
...
| Ohio | 16 | target met | statewide + Columbus, Cleveland, Dayton, Toledo, Akron, NE Ohio; Cincinnati thin |
| Tennessee | 9 | in progress | statewide + Nashville, Memphis, Knoxville/East TN; Chattanooga + more Nashville next |
...
**Rollout order (by our listing volume, playbook §7):** TX ✅ -> CA ✅ -> GA ✅ -> FL ✅ -> Indiana ✅ -> Colorado ✅ -> Ohio ✅ -> **Tennessee (in progress)** -> the 2-listing states -> the 1-listing states. Each new state gets its own `-DIRECTORY.md` built with the same rules.
